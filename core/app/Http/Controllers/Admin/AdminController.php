...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $widget['staff_delivered_orders']   = Order::whereDispatcher($admin->id)->where('payment_status', '!=', 0)->where('status',3)->count();
        $widget['staff_pending_orders']     = Order::whereDispatcher($admin->id)->where('payment_status', '!=', 0)->where('status',2)->count();
        ...
        $widget['last_seven_days']          = Order::where('payment_status', 1)->where('created_at', '>=', Carbon::today()->subDays(7))->sum('total_amount');

        $widget['last_fifteen_days']        = Order::where('payment_status', 1)->where('created_at', '>=', Carbon::today()->subDays(15))->sum('total_amount');

        $widget['last_thirty_days']         = Order::where('payment_status', 1)->where('created_at', '>=', Carbon::today()->subDays(30))->sum('total_amount');

        $widget['top_selling_products']     = Product::topSales(3);
        $widget['total_deposit_amount']     = Deposit::where('status',1)->sum('amount');
        $widget['total_deposit_amount_ngn']     = Deposit::where('status',1)->sum('final_amo');
        ...
        $depositsMonth = Order::where('created_at', '>=', Carbon::now()->subYear())
            ->where('payment_status', 1)
            ->selectRaw("SUM( CASE WHEN status = 1 THEN total_amount END) as depositAmount")
