...
{
    $curid = session()->get('syscurrency');
    
    if($curid)
    {
        $cur = Currency::whereId($curid)->first();
    }
    else
    {
        $cur = Currency::whereName('USD')->first();
        
    }
    
    $system['rate'] = $cur->rate_ngn;
    $system['rateusd'] = $cur->rate_usd;
    $system['name'] = $cur->name;
    $system['symbol'] = $cur->symbol;
...
function getPaginate($paginate = 2000000)
{
    return $paginate;
}
...
function gatewayRedirectUrlStripe()
{
   
return 'ipn.stripepaid';
 
}
function gatewayRedirectUrlStripe2()
{
   
return 'ipn.stripedeposit';
