...
<div class="featured-section padding-bottom-half padding-top-half oh">
    <div class="container">
        ...
        @foreach ($superpack as $data)
              @php
               $prod     = App\Models\Product::active()->latest()->where('status', 1)->where('category', $data->id)->inRandomOrder()->take(4)->get();
              @endphp

        @if(count($prod) > 3)
        <div class="section-header-2 ">
        <a href="{{ route('products.category', ['id'=>$data->id, 'slug'=>slug($data->name)]) }}">
        ...
        <div class="row g-4">
             ...
             @php
                if($data->offer && $data->offer->activeOffer){
                    $discount = calculateDiscount($data->offer->activeOffer->amount, $data->offer->activeOffer->discount_type, $data->base_price);
                }else $discount = 0;
                    ...
                    <div class="product-item-2-inner wish-buttons-in">
                        ...
                        <div class="product-content">
                            ...
                            <div class="product-after-content">
                                ...
                                <div class="price">
                                    @if($discount > 0)
                                    {{ systemCurrency()['symbol'] }}{{ $item->base_price - $discount/systemCurrency()['rate'] }}
